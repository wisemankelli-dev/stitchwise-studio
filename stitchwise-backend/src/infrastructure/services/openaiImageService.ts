 ...
 * Google's published pricing — ~2.3x cheaper than gpt-image-1 and billed
 * in plain per-image terms. Returns { url (data URL), buffer } or null.
 */
...
export async function generateImageWithDallE(
  ...
  // Provider strategy (2026-08-12): Google Gemini is the primary image
  // provider when GEMINI_API_KEY is configured (~$0.067/1024x1024 vs ~$0.15
  // for gpt-image-1 under per-token billing). OpenAI remains a fallback.
  if (process.env.GEMINI_API_KEY) {
    const geminiResult = await generateImageWithGemini(prompt, styleHints, userId);
